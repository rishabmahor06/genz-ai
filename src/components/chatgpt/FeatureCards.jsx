  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {items.map((it, idx) => {
        ...
        return (
          <div key={it.title} className="rounded-xl p-1.5 sm:py-4 shadow-glass-sm bg-[rgba(255,255,255,0.03)]">
            <div className={`p-2 rounded-lg inline-block bg-gradient-to-br ${gradients[idx]} text-black`}>
              <Icon />
            </div>
            <h3 className="mt-1 font-semibold text-sm">{it.title}</h3>
            <p className="mt-1 text-xs text-[color:var(--muted)]">{it.subtitle}</p>
          </div>
        );
      })}
    </div>
